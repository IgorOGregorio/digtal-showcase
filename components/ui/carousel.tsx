  ...
  ({ className, variant = "outline", size = "icon", ...props }, ref) => {
    ...
    return (
      <Button
        ...
        size={size}
        className={cn(
          "absolute h-8 w-8 rounded-full border-rose-200 bg-white/80 hover:bg-white",
...
const CarouselDots = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { itemCount: number }>(
      ...
      <div ref={ref} className={cn("flex justify-center gap-1 mt-4", className)} {...props}>
        {Array.from({ length: itemCount }).map((_, index) => (
          <Button
            ...
            size="sm"
            className={cn(
              "h-2 w-2 rounded-full p-0 border-rose-200",
              selectedIndex === index ? "bg-rose-600" : "bg-muted",
            )}
